...
- No captive-portal DNS auto-redirect in this plan (out of scope/YAGNI) — the owner browses to a documented address (the hotspot's IP) on port `8080` to reach the setup page; this matches the spec's own fallback wording ("altrimenti vai su haro.local/192.168.4.1"). Note: the actual gateway address for an `nmcli device wifi hotspot` (NetworkManager shared-mode) hotspot is `10.42.0.1`, not `192.168.4.1` (that address is the hostapd/dnsmasq convention) — corrected in the implementation and README after this was caught during final review.
